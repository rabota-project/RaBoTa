...
int main(int argc, char **argv)
{
	...
	ros::Publisher  joint_state_pub = n.advertise<sensor_msgs::JointState>("rabota/arduino/joint_states", 10);
	ros::Subscriber command_sub     = n.subscribe("rabota/arduino/command", 1000, commandCallback);
	...
	// change the angle of the motors
		msg.position[0] += 0; //(1.0/255.0)*command_input.data[0];
		msg.position[1] += 0; //(1.0/255.0)*command_input.data[1];
		msg.position[2] += 0; //(1.0/255.0)*command_input.data[2];
